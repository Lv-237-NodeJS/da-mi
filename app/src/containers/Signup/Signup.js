...
let FieldGroup = ({className, label, inErrors, ...props}) => (
  <div>
  ...
  validateField = (fieldName, value) => {
    ...
    switch (fieldName) {
      ...
      case 'password':
        ...
        inErrors.password = passwordValid ? '' : (
            'At least 6 characters, one (a-z), one (A-Z), one (0-9), one ($@$!%*?&)'
          );
      case 'confirmation':
        inErrors.confirmation = (this.state.confirmation === this.state.password) ? '' : (
  ...
  handleButtonClick = (e) => {
    ...
    .end(function(err, res) {
      if (err || !res.ok) {
        ...
        return (
          browserHistory.push('/'), 
          'You have successfully signed up! For confirmation please visit your e-mail'
        );
      }
    });
  }
